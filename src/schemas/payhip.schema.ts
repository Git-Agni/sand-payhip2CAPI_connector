import type { PayhipPaidWebhook, PayhipWebhookItem } from "../models/payhip.models.js";
...
const validatePayhipItem = (value: unknown, index: number): ValidationResult<PayhipWebhookItem> => {
...
export const validatePayhipPaidWebhook = (
  ...
  for (const field of ["id", "email", "currency", "ip_address", "payment_type"] as const) {
  ...
  const items = itemResults.flatMap((result) => (result.data ? [result.data] : []));
