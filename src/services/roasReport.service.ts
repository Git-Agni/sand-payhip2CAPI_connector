...
export function makeRoasReportService({
  ...
  async function getProductRevenue(
    ...
    const purchases = await payhipPurchaseService.getPurchasesByDateRange(
      dateRange.start,
      dateRange.end,
    );
  ...
  function formatSlackCampaignGroup(campaign: SlackCampaignGroup): string {
    if (isOrganicCampaign(campaign.campaignId)) {
      return [
        `• 🌱 Organic purchase → \n  ↳ 💰 Revenue: ${formatMoney(campaign.revenue)}  🛒 Purchases: ${campaign.purchaseCount}`,
