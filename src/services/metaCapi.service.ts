...
import type { MetaEventsRequest, MetaEventsResponse, MetaPurchaseEvent } from "../models/meta.models.js";
...
export class MetaCapiService {
  async sendPurchaseFromPayhip(payload: PayhipPaidWebhook): Promise<MetaEventsResponse> {
    ...
    const responseBody = (await response.json()) as MetaEventsResponse | { error?: unknown };
    ...
    if (!response.ok) {
      ...
      throw new Error(`Meta CAPI request failed: ${JSON.stringify(responseBody)}`);
    }
  ...
  private toPurchaseEvent(payload: PayhipPaidWebhook): MetaPurchaseEvent {
    ...
    return {
      ...
      ...(config.meta.eventSourceUrl ? { event_source_url: config.meta.eventSourceUrl } : {}),
      user_data: {
